...
// Mock data
const mockTasks = [
  { id: '1', title: 'Complete security training', category: 'Compliance', dueDate: '2024-12-15', points: 20, status: 'todo' as const },
  { id: '2', title: 'Set up development environment', category: 'IT', dueDate: '2024-12-16', points: 15, status: 'todo' as const },
  { id: '3', title: 'Meet your mentor', category: 'HR', dueDate: '2024-12-18', points: 25, status: 'todo' as const },
];

const mockModules = [
  { id: '1', title: 'Company Values & Culture', progress: 75, category: 'Company Values', totalLessons: 6, completedLessons: 4 },
  { id: '2', title: 'Information Security Basics', progress: 0, category: 'Compliance', totalLessons: 8, completedLessons: 0 },
  { id: '3', title: 'Product Overview', progress: 25, category: 'Product', totalLessons: 5, completedLessons: 1 },
];

const mockAnnouncements = [
  { id: '1', type: 'kudos', from: 'Sarah Chen', message: 'Great work on the security training!', timestamp: '2h ago' },
  { id: '2', type: 'announcement', title: 'New Learning Module Available', message: 'Leadership Fundamentals is now live', timestamp: '1d ago' },
];
...
export const Dashboard: React.FC = () => {
  ...
  const nextLevelXp = Math.floor(150 * Math.pow(user.level + 1, 1.35));
  ...
  const handleCompleteTask = (taskId: string, points: number) => {
    ...
    // In real app, would update task status via API
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-[#4A5568]">Ready to continue your learning journey?</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Progress Card - Full width on mobile, spans 2 columns on desktop */}
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl shadow-[0_6px_24px_rgba(0,0,0,0.08)] p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-[#0B2447]">Your Progress</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="text-center">
                ...
                >
                  <div className="text-center">
                    <div className="text-lg font-bold text-[#0A6ED1]">{user.level}</div>
              ...
              <div className="text-center">
                ...
                <div className="text-2xl font-bold text-[#0B2447]">{user.streakDays}</div>
              ...
              <div className="text-center">
                ...
                <div className="text-sm font-medium text-[#0B2447]">Next Badge</div>
        ...
        {/* Daily Challenge */}
        <div className="bg-gradient-to-br from-[#0A6ED1] to-[#00A0AF] rounded-2xl shadow-[0_6px_24px_rgba(0,0,0,0.08)] p-6 text-white">
        ...
        <div className="bg-white rounded-2xl shadow-[0_6px_24px_rgba(0,0,0,0.08)] p-6">
          ...
          <div className="space-y-4">
            {mockTasks.slice(0, 3).map((task) => {
              ...
              return (
                <div key={task.id} className="flex items-center gap-4 p-4 bg-[#F5F7FA] rounded-xl">
                  <button
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-[#0B2447]">{task.title}</h4>
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${dueDateInfo.className}`}>
                        {dueDateInfo.text}
                      </span>
                      <span className="text-xs text-[#4A5568]">+{task.points} XP</span>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="bg-white rounded-2xl shadow-[0_6px_24px_rgba(0,0,0,0.08)] p-6">
          ...
          <div className="space-y-4">
            {mockModules.map((module) => (
              <div key={module.id} className="p-4 border border-[#D6D9E0] rounded-xl hover:border-[#0A6ED1] transition-colors cursor-pointer">
      ...
      {/* Activity Feed */}
      <div className="bg-white rounded-2xl shadow-[0_6px_24px_rgba(0,0,0,0.08)] p-6 mt-6">
        <div className="flex items-center gap-2 mb-6">
          <Users className="w-5 h-5 text-[#0A6ED1]" />
          <h2 className="text-xl font-semibold text-[#0B2447]">Recent Activity</h2>
        </div>

        <div className="space-y-4">
          {mockAnnouncements.map((item) => (
            <div key={item.id} className="flex items-start gap-4 p-4 bg-[#F5F7FA] rounded-xl">
              ...
              <div className="flex-1">
                {item.type === 'kudos' ? (
                  <p className="text-[#0B2447]">
                    <span className="font-medium">{item.from}</span> gave you kudos: "{item.message}"
                  </p>
                ) : (
                  <>
                    <h4 className="font-medium text-[#0B2447] mb-1">{item.title}</h4>
